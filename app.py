        ...
        if face_image is not None:
            ...
            if is_face_verified:
                ...
                if is_duplicate:
                    ...
                    # Ensure DOB is in datetime format before using strftime
                    if isinstance(text_info["DOB"], str):  # If DOB is still a string
                        text_info["DOB"] = datetime.strptime(text_info["DOB"], "%Y-%m-%d")

                    # Hash the ID
                    text_info["ID"] = hash_id(text_info["ID"])

                    # Format DOB if it's in datetime
                    text_info["DOB"] = text_info["DOB"].strftime('%Y-%m-%d')

                    # Get embeddings for face image
                    text_info["Embedding"] = get_face_embeddings(face_image_path1)

                    # Insert into the database based on the ID type (PAN or Aadhar)
                    if option == "PAN":
...
def main():
    ...
    "local_db",
    type="sql",
    url="mysql://db_user:db_password@localhost:3306/kyc"
)
    wider_page()
    set_custom_theme()
